...
void enablerawmode() {
	...
	term.c_oflag &= ~(OPOST);
	term.c_cflag |= (CS8);
	term.c_lflag &= ~(ECHO | ICANON | ISIG | ISIG);
	term.c_cc[VMIN] = 0;
	term.c_cc[VTIME] = 1;
...
int editorRowCxToRx(erow *row, int cx)
{
	...
	for (j = 0; j < cx; j++) {
		...
		if (row->chars[j] == '\t')
			rx += (KILO_TAB_STOP + E.linenumoff - 1) - (rx % KILO_TAB_STOP);
		rx++;
	}
	return rx;
}
...
void editorDrawRows(struct abuf *ab) {
	...
	for (y = 0; y < E.screenrows; y++) {
		...
		if (filerow >= E.numrows) {
			...
			if (len < 0) len = 0;
			if (len > E.screencols - E.linenumoff) len = E.screencols - E.linenumoff;
...
void editorShowHelpMessage() {
	editorSetStatusMessage("Help: Ctrl-S save | Ctrl-Q quit | Ctrl-W find | Ctrl-U GOTO | Ctrl-G Help");
}
		...
		case PAGE_DOWN:
			{
				if (c == PAGE_UP || c == CTRL_KEY('y')) {
					E.cy = E.rowoff;
				} else if (c == PAGE_DOWN || c == CTRL_KEY('v')) {
